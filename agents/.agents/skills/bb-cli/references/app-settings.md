...
- `sidebar.threadGrouping.environment` decides whether sibling threads sharing
  ...
  machine and leaves them flat in Custom. Set it through Organize to Groups to
...
- Settings to Keyboard records sparse per-command chord overrides. `Mod` means
...
- `bb settings completed-turns <provider-id> <collapse|flat|default>` sets the
  ...
  default applies again. Settings to Providers has the same switch per
  provider.
...
- It shows Move server here in Settings to Machines and lets the server run
...
server gh credentials to machines; `true` enables them again. In Machines to
...
Right-click Hide moves an action into More; Settings to Appearance to Sidebar footer
...
or Settings to General to Privacy & diagnostics to Share anonymous usage data. This server-wide preference
